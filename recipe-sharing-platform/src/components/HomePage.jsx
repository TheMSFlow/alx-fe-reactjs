...
const handleNav = (id) => {
  ...
  return (
    ...
    {data.map((data) => (
     <li key={data.id} className='flex flex-col gap-4 p-8 bg-blue-100 shadow hover:shadow-lg transition-shadow duration-200 rounded-lg lg:w-[400px]' onClick={() => handleNav(data.id)}>
       ...
       <Link to={`/recipe/${data.id}`}  className='mt-4 py-4 bg-blue-950 text-white text-xl rounded-lg text-center'>View details </Link>
     </li>
    ))}
    </ul> 
    </>
